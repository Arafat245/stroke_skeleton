...
def OPA_gpu(A_t, B_t, reflect=False):
    ...
    if reflect:
        ...
        # returning numpy array
        return aligned.permute(1, 2, 0)  # Back to (32, 3, 200)
...
def log_gpu_frechet(p1, p2):
    ...
    p1_expanded = p1_expanded.permute(0, 3, 1, 2)  # (130, 200, 32, 3)
    p2 = p2.permute(0, 3, 1, 2)  # (130, 200, 32, 3)
    ...
    result = result.permute(2, 3, 1, 0)  # Back to (32, 3, 200, 130)
...
def squared_geodesic_distance(x, y, K, M, T):
    ...
    x_shapes = x_shapes.permute(0, 3, 1, 2)
    y_shapes = y_shapes.permute(0, 3, 1, 2)
...
def exp_gpu_batch(p, v):
    ...
    # geomstats creates tensors via gs.array() which uses torch default device.
    # Temporarily use CPU so all geomstats internals match our CPU tensors.
    ...
    # Force geomstats to use CPU: temporarily set default device so gs.array() creates CPU tensors
    with torch.device('cpu'):
        result = preshape.metric.exp(v_d, p_expanded_d)
    return result.permute(0, 2, 3, 1).float().to(orig_device)


def parallel_gpu(v, p1, p2, n_steps=10):
    v = v.permute(2, 0, 1)  # (32, 3, 200) to (200, 32, 3)
    p1 = p1.permute(2, 0, 1) 
    p2 = p2.permute(2, 0, 1)
