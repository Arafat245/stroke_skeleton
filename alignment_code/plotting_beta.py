...
def plotting_beta_landmark(beta, zoom = 5, playspeed=0.12, target=(0.0, 0.0, 0.0)):
    ...
    landmark_dict = beta
    timeshape = landmark_dict['LFHD'].shape[0]
...
def plot_one_traj(X, zoom = 5):
    one_traj = X.reshape(32, 3, tslen).detach().cpu().numpy()
    one_traj = one_traj.transpose(2, 0, 1).reshape(tslen, 32 * 3)  # (T, 96) for landmark_2Darray_to_dict

    kin_marker_list = ["LFHD","RFHD","LBHD","RBHD","C7","T10","CLAV","STRN","LSHO",
        "LELB","LWRA","LWRB","LFIN","RSHO","RELB","RWRA","RWRB","RFIN","LASI","RASI",
        "LTHI","LKNE","LTIB","LANK","LHEE","LTOE","RTHI","RKNE","RTIB","RANK","RHEE","RTOE"]
    ...
    ld = landmark_2Darray_to_dict(one_traj, kin_marker_list)
    p = plotting_beta_landmark(ld, zoom = zoom)